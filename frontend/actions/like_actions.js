...
export const createLike = (like) => (dispatch) => {
  return APIUtil.createLike(like).then((like) => {
    ...
    if (like.parentCommentId) dispatch(receiveChildCommentLike(like));
    else dispatch(receiveCommentLike(like));
  });
};
...
export const deleteLike = (likeId) => (dispatch) => {
  return APIUtil.deleteLike(likeId).then((like) => {
    ...
    if (like.parentCommentId) dispatch(removeChildCommentLike(like));
    else dispatch(removeCommentLike(like));
  });
};
